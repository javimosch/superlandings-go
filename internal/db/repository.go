...
func (r *SiteVersionRepository) ListVersions(siteID string) ([]SiteVersion, error) {
	...
	for rows.Next() {
		var version SiteVersion
		err := rows.Scan(&version.ID, &version.SiteID, &version.Version, &version.Path, &version.Comment, &version.Author, &version.IsActive, &version.CreatedAt)
		if err != nil {
...
// SetActiveVersion sets a version as active (deactivates others)
func (r *SiteVersionRepository) SetActiveVersion(siteID, versionID string) error {
	...
	_, err = DB.Exec(`UPDATE site_versions SET is_active = 1 WHERE id = ?`, versionID)
	if err != nil {
